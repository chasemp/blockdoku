import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css             *//* empty css             */function m(){const r=localStorage.getItem("blockdoku_lastgame"),i=document.getElementById("game-stats-content");if(!r){i.innerHTML=`
                    <div class="no-stats-message">
                        <h2>No Game Data</h2>
                        <p>You haven't completed a game yet.</p>
                        <p>Play a game to see your stats here!</p>
                    </div>
                `;return}try{const e=JSON.parse(r),g=e.prizeRecognitionEnabled!==!1;let s="";g&&e.prize&&(s+=`
                ...
                `;const t=e.breakdown||{},l={rows:e.rowClears||0,columns:e.columnClears||0,squares:e.squareClears||0};if(s+=`
                ...
                `,e.petrificationStats&&e.enablePetrification&&(s+=`
                    ...
                    `),e.speedStats&&e.speedMode!=="ignored"){const a={bonus:"Bonus Mode",punishment:"Punishment Mode",ignored:"Ignored Mode"},n=e.speedStats.averagePlacementSpeed||0,c=e.speedStats.fastestPlacement||0,o=e.speedStats.totalSpeedBonus||0,p=e.speedStats.totalPlacements||0;s+=`
                        <div class="petrification-stats" style="background: rgba(255, 165, 0, 0.08); border-color: rgba(255, 165, 0, 0.2);">
                            ...
                            <p>Total Speed Bonus: <strong>${o.toLocaleString()} points</strong></p>
                            ...
                            ${p>0?`<p>Speed Bonus per Placement: <strong>${(o/p).toFixed(1)} points</strong></p>`:""}
                        </div>
                    `}i.innerHTML=s;const d=document.getElementById("share-score-btn");d&&d.addEventListener("click",async()=>{try{const a=(e.difficulty||"NORMAL").toUpperCase(),n=window.location.origin+window.location.pathname.replace("lastgame.html","index.html"),c=`Max Streak ${e.maxCombo}, Total Combos ${e.totalCombos||0}`,o=`I scored ${e.score.toLocaleString()} in Blockdoku (${a}) — Level ${e.level}, ${e.linesCleared} lines, ${c}!`;navigator.share?await navigator.share({title:"My Blockdoku Score",text:o,url:n}):navigator.clipboard&&navigator.clipboard.writeText?(await navigator.clipboard.writeText(`${o} ${n}`),alert("Share link copied to clipboard!")):window.prompt("Copy this to share:",`${o} ${n}`)}catch(a){console.error("Share failed:",a)}})}catch(e){console.error("Error loading last game stats:",e),i.innerHTML=`
                    <div class="no-stats-message">
                        <h2>Error Loading Stats</h2>
                        <p>There was a problem loading your last game data.</p>
                    </div>
                `}}document.getElementById("new-game-btn").addEventListener("click",r=>{localStorage.setItem("blockdoku_start_new_game","true")});document.addEventListener("DOMContentLoaded",m);
